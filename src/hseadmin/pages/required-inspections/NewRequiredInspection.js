...
const NewRequiredInspection = () => {
  ...
  const onRequiredInspectionChangeHandler = (event) => {
    ...
    postRequiredInspection(postData);
  };

  const postRequiredInspection = async (data) => {
    try {
      await fetchPOST(URLConstants.REQUIRED_INSPECTION_URL, data);
  ...
  return (
    <>
      <Helmet title="Add Required Inspection" />
      ...
      <Grid justifyContent="space-between" container spacing={10}>
        <Grid item>
          ...
          <CustomBreadcrumbs aria-label="Breadcrumb" mt={2}>
            ...
            <Link component={NavLink} to="/private/required-inspections">
              Required Inspections
            </Link>
            <Typography>Add Required Inspections</Typography>
          </CustomBreadcrumbs>
        </Grid>
        <Grid item>
          <div>
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Form
            noValidate
            validated={validated}
            onSubmit={onRequiredInspectionChangeHandler}
          >
            <Row>
              <Col md={6}>
                {/* Name Field */}
...
export default NewRequiredInspection;
