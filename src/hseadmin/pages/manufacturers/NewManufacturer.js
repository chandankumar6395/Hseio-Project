...
const NewManufacturer = () => {
  ...
  const postManufacturer = async (data) => {
    try {
      await fetchPOST(URLConstants.MANUFACTURERS_URL, data);
      ...
      <Grid justifyContent="space-between" container spacing={10}>
        <Grid item>
          ...
          <CustomBreadcrumbs aria-label="Breadcrumb" mt={2}>
            ...
            <Link component={NavLink} to="/private/manufacturers">
              Manufacturer
            </Link>
            <Typography>Add Manufacturer</Typography>
          </CustomBreadcrumbs>
        </Grid>
        <Grid item>
          <div>
      ...
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Form
            noValidate
            validated={validated}
            onSubmit={onManufacturerChangeHandler}
          >
            <Row>
              <Col md={6}>
                {/* Name Field */}
