...
const RequiredCdlEndorsement = () => {
  ...
  const fetchRequiredCdlEndorsement = async () => {
    try {
      setLoading(true);
      const response = await fetchGET(
        `${URLConstants.GET_REQUIRED_CDL_ENDORSEMENT_URL}?limit=100&page=1`
      );
  ...
  return (
    <>
      <Helmet title="Required CDL Endorsements" />
      ...
      <Grid justifyContent="space-between" container spacing={10}>
        <Grid item>
          ...
          <Breadcrumbs aria-label="Breadcrumb" mt={2}>
            ...
            <Typography>Required Cdl Endorsement</Typography>
          </Breadcrumbs>
        </Grid>
        <Grid item>
          <div>
            <NavLink to="/private/required-cdl-endorsements/add">
              <Button variant="contained" color="primary">
                <AddIcon />
                Add Required Cdl Endorsement
              </Button>
            </NavLink>
          </div>
        </Grid>
...
export default RequiredCdlEndorsement;
